...
app.post('/api/setup/write-config', (req, res) => {
    const { domain, project, uiPort, vscPort, ghId, ghSecret, ghAdmin, ghCb,
            gitName, gitEmail, githubUser, claudeKey } = req.body;
    ...
    const localLines = [
        ...
        domain      ? `GAMADCODE_DOMAIN="${domain}"` : 'GAMADCODE_DOMAIN=""',
    ...
    const existingSecrets = fs.existsSync(secretsEnv)
        ? fs.readFileSync(secretsEnv, 'utf8') : '';
